...
class RGDTLayer(nn.Module):
    ...
    def forward(self, graph, ent_feat, rel_feat, get_attention=False):
        with graph.local_scope():
            ...
            in_head = in_dst = self.in_feat_drop(self.graph_layer_norm(ent_feat))
            in_rel = self.in_feat_drop(self.graph_layer_norm(rel_feat))
            ...
            feat_rel = self.fc_rel(in_rel).view(-1, self._num_heads, self._head_dim)
            ...
            e = (e * self.attn).sum(dim=-1).unsqueeze(dim=2)/math.sqrt(self._head_dim)  # (num_edge, num_heads, 1)
            # compute softmax
            if self.ppr_diff:

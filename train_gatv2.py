...
from baselines.gatv2 import GATv2
...
def evaluate(g, model,  features, labels, mask, debug=False):
    model.eval()
    with torch.no_grad():
...
def gat_error_analysis(graph, model, features, labels, val_mask, test_mask, train_mask):
    ...
    test_acc, test_predictions, test_true_labels = evaluate(graph, model, features, labels, test_mask, debug=True)
    print("Test Accuracy {:.4f}".format(test_acc))
    valid_acc, valid_predictions, valid_true_labels = evaluate(graph, model, features, labels, val_mask, debug=True)
    print("Valid Accuracy {:.4f}".format(valid_acc))
    train_acc, train_predictions, train_true_labels = evaluate(graph, model, features, labels, train_mask, debug=True)
